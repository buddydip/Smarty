...
int mq2 = A0;             // Connect Gas sensor to analog pin A0
...
void setup() {
  pinMode(mq2, INPUT);           // Define MQ2 pin as input
...
void loop() {
  int mq2Value = analogRead(mq2); // Read sensor value
  if (mq2Value >= threshold) {    // Check if it exceeds the threshold value
